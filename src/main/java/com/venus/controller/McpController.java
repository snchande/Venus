 ...
 *
 * Available tools:
 *   venus_execute_code       — Execute Java code in a JShell session
 *   venus_list_notebooks     — List all available notebooks
 *   venus_read_notebook      — Read all cells from a notebook
 *   venus_run_pipeline       — Execute a pipeline cell with dependency resolution
 *   venus_search_cells       — Search for cells by anchor name or source content
 *   venus_load_module        — Load a named cell module from a notebook into a session
 *   venus_create_notebook    — Create a new notebook, optionally pre-populated with cells
 *   venus_append_cell        — Append a new cell to an existing notebook and optionally execute it
 */
@RestController
...
public class McpController {
    ...
    private static final String SERVER_NAME    = "venus-notebooks";
    ...
    private Map<String, Object> handleToolsList() {
        List<Map<String, Object>> tools = List.of(
            toolDef("venus_execute_code",
                "Execute Java code in a Venus JShell session and return output",
                Map.of(
                    "type", "object",
                    "properties", Map.of(
            ...
            toolDef("venus_list_notebooks",
                "List all available notebooks in Venus",
                Map.of("type", "object", "properties", Map.of())
            ),
            toolDef("venus_read_notebook",
            ...
            toolDef("venus_create_notebook",
                "Create a new Venus notebook, optionally pre-populated with cells",
                Map.of(
                    "type", "object",
                    "properties", Map.of(
    ...
    private Map<String, Object> handleToolsCall(Map<String, Object> params, String mcpSessionId)
        ...
        String text = switch (toolName) {
            case "venus_execute_code"   -> toolExecuteCode(args);
            case "venus_list_notebooks" -> toolListNotebooks();
            case "venus_read_notebook"  -> toolReadNotebook(args);
            case "venus_run_pipeline"   -> toolRunPipeline(args);
            case "venus_search_cells"   -> toolSearchCells(args);
            case "venus_load_module"    -> toolLoadModule(args);
            case "venus_create_notebook" -> toolCreateNotebook(args);
            case "venus_append_cell"    -> toolAppendCell(args);
    ...
    private String toolLoadModule(Map<String, Object> args) throws McpException {
        ...
        com.venus.model.Cell syntheticCell = new com.venus.model.Cell();
        syntheticCell.setId("__mcp_load_cell__");
        syntheticCell.setType(com.venus.model.CellType.CODE);
    ...
    private String toolCreateNotebook(Map<String, Object> args) throws McpException {
        ...
        com.venus.model.Notebook notebook = notebookService.createNotebook(name, userService.getLocalUserId());
        ...
        for (Map<String, Object> cellDef : cellDefs) {
            com.venus.model.Cell cell = new com.venus.model.Cell();
            ...
            cell.setType("MARKDOWN".equalsIgnoreCase(cellType)
                    ? com.venus.model.CellType.MARKDOWN : com.venus.model.CellType.CODE);
            cell.setMode("jshell");
    ...
    private String toolAppendCell(Map<String, Object> args) throws McpException {
        ...
        com.venus.model.Notebook notebook = notebookService.getNotebook(notebookId, userService.getLocalUserId()).orElse(null);
        if (notebook == null) {
            throw new McpException(-32602, "Notebook not found: " + notebookId);
        }
        ...
        cell.setId(cellId);
        cell.setType(isMarkdown ? com.venus.model.CellType.MARKDOWN : com.venus.model.CellType.CODE);
        cell.setMode("jshell");
