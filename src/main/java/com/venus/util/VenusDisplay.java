...
/**
 * Venus Notebooks — display helper available in every JShell session.
 *
 * Charts:
 *   XYChart   c = VenusDisplay.xyChart("Title","X","Y"); c.addSeries(...); VenusDisplay.show(c);
 *   CategoryChart b = VenusDisplay.barChart("Title","X","Y"); ...
 *   PieChart  p = VenusDisplay.pieChart("Title"); ...
 *
 * DataFrame (Tablesaw):
 *   Table df = VenusDisplay.loadCsv("data/sales.csv");
 *   VenusDisplay.show(df);          // renders as HTML table (first 50 rows)
 *   VenusDisplay.show(df, 20);      // first 20 rows
 *   VenusDisplay.info(df);          // column types + non-null counts
 *   VenusDisplay.describe(df);      // numeric summary statistics per column
 *
 * Statistics:
 *   VenusDisplay.stats("label", double[]);
 *   VenusDisplay.table(String[][], headers...);
 */
public class VenusDisplay {
     ...
     * Path can be absolute or relative to the Venus working directory.
     */
    ...
    public static void show(tech.tablesaw.api.Table table, int maxRows) {
        ...
        StringBuilder sb = new StringBuilder("VENUS_HTML:");
        sb.append("<div class=\"venus-df-wrap\">");
        sb.append("<div class=\"venus-df-meta\">")
          .append(escHtml(table.name())).append("  ")
          .append(table.rowCount()).append(" rows × ")
          .append(table.columnCount()).append(" cols");
        if (truncated) sb.append("  <span class=\"venus-df-trunc\">(showing first ").append(rows).append(")</span>");
        sb.append("</div>");
        sb.append("<table class=\"venus-table venus-df\">");
        sb.append("<thead><tr>");
        for (var col : table.columns()) {
            sb.append("<th>").append(escHtml(col.name()))
              .append("<span class=\"venus-col-type\">").append(escHtml(col.type().name())).append("</span>")
              .append("</th>");
        }
    ...
    public static void table(String[][] data, String... headers) {
        StringBuilder sb = new StringBuilder("VENUS_HTML:");
        sb.append("<table class=\"venus-table\">");
        if (headers.length > 0) {
            sb.append("<thead><tr>");
    ...
    private static void renderChart(Chart chart) {
        ...
        catch (Exception e) { System.err.println("VenusDisplay.show() failed: " + e.getMessage()); }
    }
    ...
    private static void printImage(BufferedImage img) {
        try {
            ...
            System.out.println("VENUS_IMG:data:image/png;base64,"
                + Base64.getEncoder().encodeToString(baos.toByteArray()));
        } catch (Exception e) { System.err.println("VenusDisplay image error: " + e.getMessage()); }
    }
