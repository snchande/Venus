...
import com.venus.model.NuGetPackageInfo;
 ...
 *
 * <h3>Venus helpers (C#)</h3>
 * <pre>
 *   VenusHtml("&lt;b&gt;bold&lt;/b&gt;");    // rendered as inline HTML in output
 *   VenusDisplay(myObject);          // Console.WriteLine
 *   VenusTable(myList);              // ASCII table
 * </pre>
 *
 * <h3>Venus helpers (F#)</h3>
 * <pre>
 *   venusHtml "&lt;b&gt;bold&lt;/b&gt;"    // rendered as inline HTML
 *   venusDisplay myObj               // printfn "%A"
...
public class DotNetExecutionService {
    ...
    // C# preamble: standard usings + Venus helper methods
    private static final String CSHARP_PREAMBLE =
        ...
        "using System.Text;\n\n" +
        "// Venus display helpers\n" +
        "static void VenusHtml(string content) =>\n" +
        "    Console.WriteLine(\"VENUS_HTML:\" + content.Replace(\"\\n\", \" \"));\n\n" +
        "static void VenusDisplay(object obj) => Console.WriteLine(obj);\n\n" +
        "static void VenusTable<T>(IEnumerable<T> items) {\n" +
    ...
    // F# preamble: standard opens + Venus helper functions
    private static final String FSHARP_PREAMBLE =
        "open System\n" +
        "open System.Linq\n" +
        "open System.Collections.Generic\n\n" +
        "// Venus display helpers\n" +
        "let venusHtml (content: string) =\n" +
        "    printfn \"VENUS_HTML:%s\" (content.Replace(\"\\n\", \" \"))\n\n" +
        "let venusDisplay obj = printfn \"%A\" obj\n\n" +
        "let venusTable (data: 'a list) =\n" +
    ...
    public ExecutionResult executeCSharp(String sessionId, String cellId, String code) {
        ...
        if (dotnet == null) {
            return err(sessionId, cellId,
                ...
                "  - Restart Venus after installing", start);
        }

        Path tempDir = null;
        try {
            tempDir = Files.createTempDirectory("venus-cs-");

            // Generate .csproj with NuGet package references
            String targetFramework = detectDotNetTargetFramework(dotnet);
            Files.writeString(tempDir.resolve("venus-cell.csproj"),
            ...
            if (!depends.isEmpty()) {
                ...
                if (!contextStmts.toString().isBlank()) {
                    programContent.append("var __venusCtxOut = Console.Out;\n");
                    programContent.append("Console.SetOut(TextWriter.Null);\n");
                    programContent.append(contextStmts);
                    programContent.append("Console.SetOut(__venusCtxOut);\n");
    ...
    public ExecutionResult executeFSharp(String sessionId, String cellId, String code) {
        ...
        Path tempDir = null;
        try {
            tempDir = Files.createTempDirectory("venus-fs-");
            ...
            if (!depends.isEmpty()) {
                ...
                if (!contextCode.toString().isBlank()) {
                    scriptContent.append("let __venusCtxOut = System.Console.Out\n");
                    scriptContent.append("System.Console.SetOut(System.IO.TextWriter.Null)\n");
                    scriptContent.append(contextCode);
                    scriptContent.append("System.Console.SetOut(__venusCtxOut)\n");
