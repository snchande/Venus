...
> **Interactive multi-language notebooks in your browser — Java, C++, C#, F#, JavaScript — with AI assistance**
...
Venus Notebooks is a **locally-hosted, browser-based notebook environment** — like Jupyter, but for six languages. Write and run Java, JShell, JavaScript, C#, F#, and C++ cells side by side, manage Maven, npm, and NuGet dependencies, chain cells with pipeline orchestration, and get AI assistance from Claude, GitHub Copilot, or Gemini — all without leaving your browser.
...
| Feature | Description |
|---------|-------------|
| **Six execution modes** | JShell · Java · JavaScript (Node.js) · C# · F# · C++ |
...
| **npm Package Manager** | Install npm packages for JavaScript cells with one click |
...
| **Pipeline Orchestration** | Chain cells with `//@ depends:` annotations — works across all 6 languages |
...
| **Tutorial Library** | 23 built-in tutorials across JShell, Java, JavaScript, C#, F#, and C++ |
...
| **Node.js** | 18+ | Optional — for JavaScript cells and npm packages |
...
> **Windows users**: `venus.cmd` CLI handles everything — build, start, stop, and browser open in one command.
...
venus
```
`venus.cmd` detects if a JAR exists, builds it if needed, starts the server, and opens your browser automatically.
...
The `venus` CLI opens this automatically on Windows.
...
Each code cell has a **mode button** (top-right of the cell) that cycles between all six languages:
...
Venus ships with 23 built-in tutorials. Open the Notebook Browser and click **Venus Tutorials**.
...
`venus.cmd` in the project root is a full command-line interface:
...
| `venus start --bg` | Start in background; logs to `venus.log` |
| `venus stop` | Stop the running server |
| `venus status` | Show running state, PID, Java version |
| `venus build` | Build JAR (skips if already built) |
| `venus rebuild` | Force clean rebuild |
| `venus open` | Open browser (server must be running) |
| `venus logs` | Tail background logs |
| `venus help` | Show help screen |
...
│   ├── service/             # Business logic (JShell, Java, Node.js, C#, F#, C++, AI, Maven, npm)
...
│   └── start.bat            # Windows fallback launcher
├── venus.cmd                # Full Windows CLI
└── docs/
