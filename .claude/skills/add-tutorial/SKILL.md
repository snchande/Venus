---
name: add-tutorial
description: Create a new Venus tutorial notebook (.vnb) that conforms to the built-in tutorial library format. Use when the user wants to ship a new lesson — JShell-301, TS-501, etc. Generates a properly-shaped .vnb JSON, registers it with NotebookService, and updates README.md tutorial table.
---
...
The `.vnb` is a JSON file matching `com.venus.model.Notebook`. Look at `notebooks/tutorials/js-101.vnb` as the canonical template.
...
3. **Self-contained** — no external package installs required beyond what Venus ships pre-loaded.
...
6. Increment the tutorial count in `README.md` ("Venus ships with N built-in tutorials") and in the brochure (`docs/brochure/venus-brochure.html`).
7. Run `venus rebuild && venus start` and verify the tutorial appears in **Notebook Browser → Venus Tutorials** and renders end-to-end.
